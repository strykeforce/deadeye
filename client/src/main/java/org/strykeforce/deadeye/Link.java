...
class Link extends Thread {
  ...
  static InetAddress getOwnAddress(List<InetAddress> addresses) {
    if (addresses.size() != 1) throw new RuntimeException("Multiple Inet Addresses: " + addresses);
  ...
  static boolean hasLinkAddresses(
      @NotNull List<InetAddress> addresses, @NotNull List<Config> configs) throws IOException {
  ...
  public void run() {
    try {
      ...
      while (true) {
        ...
        if (handler != null) handler.handleTargetData(buffer);
        else logger.warn("unrecognized target data id: {}", id);
      }
    } catch (IOException e) {
