 ...
 * <ul>
 *   <li>{@link UprightRectTargetData} when using an {@code UprightRectPipeline}</li>
 *   <li>{@link MinAreaRectTargetData} when using a {@code MinAreaRectPipeline}</li>
 *   <li>{@link TargetListTargetData} when using a {@code TargetListPipeline}</li>
 * </ul>
 *
 ...
 * </pre></blockquote>
 *
 * The recommended way of using the {@code TargetData} sent by the Deadeye camera pipeline in a {@code Subsystem} is:
 *
 * <blockquote>
 * <pre>
 * public void periodic() {
 *   UprightRectTargetData td = deadeye.getTargetData();
 *   if (td != null)
 *     processTargetData(td);
 *   }
 * </pre></blockquote>
 *
 * The {@link Deadeye#getTargetData()} method will return {@code null} if valid target data was not received since the last
 * call to it.
 */
...
public class Deadeye<T extends TargetData> {
    ...
    private final AtomicReference<T> targetData = new AtomicReference<>();
     ...
     *                    type.
     * @param linkAddress the IP address the Deadeye daemon should send data to. If null, this
     *                    client
    ...
    /**
     * Constructs an instance of {@code Deadeye} and initializes a connection to a Deadeye camera
     * using a specified NetworkTables instance while manually specifying this client IP address.
     * This
    ...
    public Deadeye(
        ...
        try {
            ...
            targetData.set(constructor.newInstance());
            jsonAdapter = targetData.get().getJsonAdapter();
     ...
     * time {@code TargetData} is received. This method will be called from a separate thread so care
     * must be taken to avoid concurrency issues.
     *
     * <B>Using this method for target data access is only needed in
     * advanced cases and should generally be avoided in favor of {@link Deadeye#getTargetData()}.</b>
     *
    ...
    void handleTargetData(BufferedSource source) throws IOException {
        var td = jsonAdapter.fromJson(source);
        targetData.set(td);
        targetDataListener.onTargetData(td);
    }

    /**
     * Gets the most recent {@code TargetData} received by this {@code Deadeye} instance. This will return {@code null}
     * if the target data has not been refreshed since the last call to {@code getTargetData}.
     *
     * @return a target data update or null if not updated.
     */
    @Nullable
    public T getTargetData() {
        return targetData.getAndSet(null);
    }
    ...
    /**
     * Gets camera light on or off state. This value may lag actual state if called immediately
     * after
